...
class GMMNoiseFilter:
    ...
    def __init__(self, input_file_name: str, selection_mode: int, data: np.ndarray):
    ...
    def save_noise_level_plot(self, folder_path: str) -> None:
        ...
        delta_num_peaks = self._input_data.shape[0] - self._denoised_data.shape[0]
        delta_np_percent = (delta_num_peaks / self._input_data.shape[0]) * 100

        plt.figure(figsize=(6, 3), dpi=300)

        _, stems, _ = plt.stem(
            self._input_data[:, 0], self._input_data[:, 1], markerfmt=" "
        )
    ...
    def save_bic_plot(self, folder_path: str) -> None:
        ...
        plt.plot(x, self._bics, color="black", marker="o", linewidth=2, markersize=3)

        plt.plot(
            [self._optimal_n_clust],
        ...
        plt.title(
            "BIC values for different numbers of Gaussians\n"
            + f"{self._file_name}\n"
    ...
    def _get_intensity_threshold(self) -> float:
        ...
        gmm = GMM(
            n_components=self._optimal_n_clust,
            max_iter=1000,
            random_state=135,
        ...
        for i in range(means.shape[0]):
            ...
            y = norm.pdf(
                self._x_axis, float(means[i][0]), np.sqrt(float(covs[i][0][0]))
            )
            y *= weights[i]
    ...
    def _get_intersection_x(
        self, m1: float, m2: float, std1: float, std2: float, w1: float, w2: float
    ) -> float:
    ...
    def _generate_bic_values(self, max_clusters: int) -> None:
        ...
        for i in range(1, max_clusters + 1):
            ...
            gmm = GMM(
                n_components=i, max_iter=1000, random_state=42, covariance_type="full"
            )
    ...
    def _gen_hist_plot(self) -> None:
        ...
        plt.plot(
            ...
            c=f"C{i + 1}",
            ls="dashed",
            label="Sum of gaussins",
            zorder=-1,
        )
        plt.vlines(
            x=self._intersect_x,
            ymin=0,
            ymax=np.max(self._gaussian_sum),
            lw=2,
            color="magenta",
            zorder=50,
            label="Noise threshold",
        ...
        plt.text(
            x=self._intersect_x + 0.1,
            y=np.max(self._gaussian_sum) / 2,
            s=f"{self._intersect_x:.3f}",
