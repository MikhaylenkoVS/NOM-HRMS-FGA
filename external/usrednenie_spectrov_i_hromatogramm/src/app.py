...
class Application(tk.Frame):
    ...
    def _create_widgets(self) -> None:
        ...
        # Open a file ---------------------------------------------------------
        raw_file_button = tk.Button(
            self, text="Open .RAW file", command=self._ask_raw_file, bg="#c6b4ed"
        )
        ...
        # Ranges --------------------------------------------------------------
        start_time_label = tk.Label(self, text="Start time, min", justify=tk.CENTER)
        step_time_label = tk.Label(self, text="Time step, min", justify=tk.CENTER)
        end_time_label = tk.Label(self, text="End time, min", justify=tk.CENTER)
        ...
        output_folder_button = tk.Button(
            ...
            command=self._get_output_folder,
            bg="#c6b4ed",
        )
        self._output_folder_entry = ttk.Entry(
            self, width=30, justify=tk.LEFT, state="readonly"
        )

        output_folder_button.grid(row=3, column=0, sticky="nsew")
        self._output_folder_entry.grid(row=3, column=1, columnspan=2, sticky="nsew")

        # Noise removal -------------------------------------------------------
        apply_noise_filter_check = tk.Checkbutton(
            self, text="Apply GMM noise filter", variable=self._apply_noise_filter
        )
        save_noise_level_plot_check = tk.Checkbutton(
            self, text="Save noise level plots", variable=self._save_noise_level_plot
        )
        save_histogram_plot_check = tk.Checkbutton(
            self, text="Save histogram plots", variable=self._save_histogram_plot
        )
        ...
        self.run_button = tk.Button(
            self, text="Run!", height=2, command=self._run, bg="#aaefa2"
        )
    ...
    def _run(self) -> None:
        ...
        self._output_path = os.path.join(
            self._output_folder_entry.get(), self._current_file_name
        )

        if not os.path.exists(self._output_path):
            os.mkdir(self._output_path)

        self._plot_folder_path = os.path.join(self._output_path, "plots")

        if (
            (self._save_noise_level_plot.get() or self._save_histogram_plot.get())
        ...
        self._end_time = (
            float(self._end_time_entry.get()) if self._end_time_entry.get() else -999.0
        )
        ...
        if self._step_time == 0:
            ...
            for st in np.arange(self._start_time, self._end_time, self._step_time):
                ...
                self._average_one_time_range(current_start_time, current_end_time)
    ...
    def _save_peak_lists(
        self, spectrum_lists: dict, start_time: float, end_time: float
    ) -> None:
        """ """
        ...
        for list_name, spectrum_list in spectrum_lists.items():
            ...
            save_file_path = os.path.join(self._output_path, save_file_name)

            self._save_csv_file(save_file_path + ".csv", spectrum_list)
            self._save_tab_separated_file(save_file_path + ".txt", spectrum_list)
    ...
    def _print_checkbox_values(self) -> None:
        ...
        print(
            "Apply GMM noise filter:",
            clr.Fore.MAGENTA,
            self._apply_noise_filter.get(),
            clr.Style.RESET_ALL,
        )
        print(
            "Save noise level plots:",
            clr.Fore.MAGENTA,
            self._save_noise_level_plot.get(),
            clr.Style.RESET_ALL,
        )
        print(
            "Save histogram plots:",
            clr.Fore.MAGENTA,
            self._save_histogram_plot.get(),
            clr.Style.RESET_ALL,
        )
    ...
    def _update_time_values(self) -> None:
        ...
        self._start_time = (
            self._start_time if self._start_time > self._min_rt else self._min_rt
        )
        self._step_time = self._step_time if self._step_time > 0 else 0
        self._end_time = (
            self._end_time if self._end_time < self._max_rt else self._max_rt
        )

    def _average_one_time_range(self, start_time: float, end_time: float) -> None:
        """ """
