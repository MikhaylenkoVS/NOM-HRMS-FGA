...
def _style(root: tk.Tk) -> ttk.Style:
    s = ttk.Style(root)
    s.theme_use("clam")
    s.configure(".", background=BG, foreground=FG, font=FONT, fieldbackground=PANEL)
    ...
    s.configure(
        "Treeview", background=PANEL, foreground=FG, fieldbackground=PANEL, rowheight=22
    )
    s.configure(
        "Treeview.Heading",
        background=BTN,
    ...
    s.configure("TScrollbar", background=BTN, troughcolor=PANEL, arrowcolor=FG)
