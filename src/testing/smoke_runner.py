...
def _run_one_set(set_dir: Path, output_dir: Path) -> SetSmokeResult:
    ...
    try:
        ...
        _p = PIPELINE_PARAMS
        src_sp = load_spectrum(
            str(orig_csv), mass_min=_p["load_mass_min"], mass_max=_p["load_mass_max"]
        )
        src_sp = denoise(
            src_sp, force=_p["noise_force"], intensity=_p["noise_intensity"]
        )
        dmet_sp = load_spectrum(
            str(dmet_csv), mass_min=_p["load_mass_min"], mass_max=_p["load_mass_max"]
        )
        dmet_sp = denoise(
            dmet_sp, force=_p["noise_force"], intensity=_p["noise_intensity"]
        )
        dacet_sp = load_spectrum(
            str(dacet_csv), mass_min=_p["load_mass_min"], mass_max=_p["load_mass_max"]
        )
        dacet_sp = denoise(
            dacet_sp, force=_p["noise_force"], intensity=_p["noise_intensity"]
        )
        ...
        df_dmet = find_series(
            ...
            delta=DELTA_CD3,
            ppm_tol=_p["ppm_tol"],
            max_groups=_p["max_groups"],
            allow_gaps=_p["allow_gaps"],
        )
...
def _export_summary(suite: SmokeSuiteResult, output_root: Path):
    ...
    for s in suite.sets:
        set_sum = {
            ...
            "compounds_with_structures": sum(
                1 for c in s.compound_results if c.structures_found > 0
            ),
        }
