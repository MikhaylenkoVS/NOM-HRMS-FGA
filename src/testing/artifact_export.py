...
def load_spectrum_csv(path: str) -> pd.DataFrame:
    ...
    if "mass" not in df.columns or "intensity" not in df.columns:
        raise ValueError(
            f"CSV {path} не содержит mass/intensity. Колонки: {df.columns.tolist()}"
        )
...
def plot_three_spectra(
    ...
    dacet_path: str,
    save_path: Path,
    title: str = "Mass Spectra Overlay",
):
    ...
    for ax, df, color, label in zip(axes, [orig, dmet, dacet], colors, labels):
        ax.vlines(
            df["mass"], 0, df["intensity"], colors=color, linewidth=0.8, alpha=0.8
        )
...
def plot_series_grid(
    ...
    src = Spectrum(
        table=pd.DataFrame(
            {"mass": df_series["mass_src"].values, "intensity": np.ones(len(df_series))}
        )
    )
    deriv = Spectrum(
        table=pd.DataFrame(
            {"mass": deriv_mz_array, "intensity": np.ones(len(deriv_mz_array))}
        )
    )
    _vis_series(
        src,
...
def plot_histogram(df: pd.DataFrame, column: str, save_path: Path, title: str = ""):
    ...
    """
    vals = _safe_df(df)[column].dropna().astype(int)
    if vals.empty:
        return
    fig, ax = plt.subplots(figsize=(6, 4))
    ax.hist(
        vals, bins=range(vals.max() + 2), color="#a6e3a1", edgecolor="black", alpha=0.85
    )
