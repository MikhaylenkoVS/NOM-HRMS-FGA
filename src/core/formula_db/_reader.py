...
from ._packed import (
    MASSES, MASS_U, MASS_SCALE, BIN_WIDTH_U, RECORD_SIZE,
    unpack_c_n_o_s_p, byte_unshuffle_uint32_le,
    restore_h, decode_block, formula_to_string, calculate_exact_mass,
    dbe_from_counts,
)
...
class LRUBlockCache:
    def __init__(self, max_blocks=8):
        ...
        self._cache: collections.OrderedDict[tuple, list[dict]] = collections.OrderedDict()
...
class FormulaDatabaseReader:
    def __init__(self, manifest_path: str | Path, cache_size=8, verify=True):
        ...
        name = self._mp.name
        fdb_name = name[:-len(".manifest.json")] + ".fdb" if name.endswith(".manifest.json") else self._mp.stem + ".fdb"
    ...
    @property
    def total_formulas(self): return self._manifest["formula_count"]
    @property
    def max_mass(self): return self._max_mass_u / MASS_SCALE

    def _open(self):
        if self._file is None: self._file = open(self._fdb, "rb")
    ...
    def _load_block(self, bin_id: int) -> list[dict]:
        ...
        if len(shuffled) != bm["raw_size"]:
            raise RuntimeError(f"Block {bin_id}: expected {bm['raw_size']} raw, got {len(shuffled)}")
    ...
    def search(self, target_mass: float, ppm=1.0,
               element_filter: dict[str, tuple[int, int]] | None = None,
               max_results=100) -> list[SearchResult]:
        ...
        for bi in bin_range:
            if bi not in self._blocks: continue
            ...
            for cts in formulas:
                ...
                if err > delta_u: continue
                err_ppm = (err / target_u) * 1e6

                if element_filter:
                    if any(cts.get(el, 0) < lo or cts.get(el, 0) > hi
                           for el, (lo, hi) in element_filter.items()):
                        continue

                results.append(SearchResult(
                    fstr, dict(cts), mass_u / MASS_SCALE, err_ppm, dbe_from_counts(cts)))
                seen.add(fstr)
