...
def pack_c_n_o_s_p(c=0, n=0, o=0, s=0, p=0) -> int:
    ...
    for el, (shift, bits, mask) in _ELEMENT_BITS.items():
        val = vals[el]
        if val < 0:
            raise ValueError(f"Negative {el}: {val}")
        if val > mask:
            raise ValueError(f"{el}={val} exceeds {bits}-bit limit (max {mask})")
...
def restore_h(
    ...
    if not (block_low_u <= mass_u < block_high_u):
        return None
    if mass_u > max_mass_u:
        return None
    dbe_num = 2 * c + 2 + n + p - h
    if dbe_num < 0 or dbe_num % 2 != 0:
        return None
    return h, mass_u
...
def byte_shuffle_uint32_le(data: bytes) -> bytes:
    ...
    if len(data) % 4:
        raise ValueError(f"len must be multiple of 4, got {len(data)}")
    n = len(data) // 4
...
def byte_unshuffle_uint32_le(data: bytes) -> bytes:
    ...
    for i in range(n):
        off = i * 4
        r[off : off + 4] = bytes(
            [data[i], data[n + i], data[2 * n + i], data[3 * n + i]]
        )
    return bytes(r)
...
def decode_block(
    ...
    if len(raw) % RECORD_SIZE:
        raise ValueError(f"Unaligned: {len(raw)}")
    ...
    for i in range(n):
        ...
        if elem_filter:
            ...
            if any(
                actual.get(el, 0) < lo or actual.get(el, 0) > hi
        ...
        h, mass_u = r
        results.append(
            {"C": c, "N": nv, "O": o, "S": sv, "P": pv, "H": h, "mass_u": mass_u}
        )
    return results
