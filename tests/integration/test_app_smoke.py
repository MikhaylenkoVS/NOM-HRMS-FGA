...
def test_smoke_all_sets(data_dir, smoke_output_dir):
    ...
    for s in suite.sets:
        assert (
            s.pipeline_success
        ), f"{s.set_name}: пайплайн завершился с ошибкой: {s.error}"
        assert s.success, f"{s.set_name}: общий провал: {s.error}"
        assert (
            s.result_table_path is not None
        ), f"{s.set_name}: result_table.csv не создан"
        assert (
            s.result_table_path.exists()
        ), f"{s.set_name}: result_table.csv отсутствует"
